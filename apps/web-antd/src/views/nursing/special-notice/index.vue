...
function openCreateModal() {
  ...
  resetForm();
  // 特殊情况告知书固定宣教类型为 5
  formModel.value.educationType = 5;
...
async function openEditModal(record: CustomerEducationItem) {
  ...
  try {
    ...
    formModel.value = {
      ...
      // 编辑时沿用接口返回的宣教类型，一般为 5
      educationType: detail.educationType ?? 5,
...
async function handleSubmit() {
  try {
    ...
    const baseData: CreateCustomerEducationParams = {
      ...
      // 创建特殊情况告知书时固定为 5，编辑时保留原值
      educationType: isEditMode.value ? values.educationType : 5,
...
<template>
  <div class="p-4">
    ...
    <Modal
      ...
      :title="isEditMode ? '更新特殊情况告知书' : '创建特殊情况告知书'"
      width="700px"
