...
function openCreateModal() {
  ...
  resetForm();
  // 健康告知书固定宣教类型为 4
  formModel.value.educationType = 4;
...
async function openEditModal(record: CustomerEducationItem) {
  ...
  try {
    ...
    formModel.value = {
      ...
      // 编辑时沿用接口返回的宣教类型，一般为 4
      educationType: detail.educationType ?? 4,
...
async function handleSubmit() {
  try {
    ...
    const baseData: CreateCustomerEducationParams = {
      ...
      // 创建健康告知书时固定为 4，编辑时保留原值
      educationType: isEditMode.value ? values.educationType : 4,
...
<template>
  <div class="p-4">
    ...
    <Modal
      ...
      :title="isEditMode ? '更新健康告知书' : '创建健康告知书'"
      width="700px"
