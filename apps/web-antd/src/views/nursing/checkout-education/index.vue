...
function openCreateModal() {
  ...
  resetForm();
  // 护理部出所宣教固定宣教类型为 3
  formModel.value.educationType = 3;
...
async function openEditModal(record: CustomerEducationItem) {
  ...
  try {
    ...
    formModel.value = {
      ...
      educationType: detail.educationType ?? 3,
...
async function handleSubmit() {
  try {
    ...
    const baseData: CreateCustomerEducationParams = {
      ...
      // 护理部出所宣教固定为 3
      educationType: isEditMode.value ? values.educationType : 3,
...
<template>
  <div class="p-4">
    ...
    <Modal
      ...
      :title="isEditMode ? '更新护理部出所宣教' : '创建护理部出所宣教'"
      width="700px"
