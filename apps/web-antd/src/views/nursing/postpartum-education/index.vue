...
function openCreateModal() {
  ...
  resetForm();
  // 产后护理卫教固定宣教类型为 2
  formModel.value.educationType = 2;
...
async function openEditModal(record: CustomerEducationItem) {
  ...
  try {
    ...
    formModel.value = {
      ...
      // 编辑时沿用接口返回的宣教类型，一般为 2
      educationType: detail.educationType ?? 2,
...
async function handleSubmit() {
  try {
    ...
    const baseData: CreateCustomerEducationParams = {
      ...
      // 创建产后护理卫教时固定为 2，编辑时保留原值
      educationType: isEditMode.value ? values.educationType : 2,
...
<template>
  <div class="p-4">
    ...
    <Modal
      ...
      :title="isEditMode ? '更新产后护理卫教' : '创建产后护理卫教'"
      width="700px"
